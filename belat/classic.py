...
class Scheme(bs.Scheme):
    ...
    gram_baza = {
        "l_galosn_lit": l_galosn_lit,
        "l_zychn_lit": l_zychn_lit,
        "l_vialik_lit": l_vialik_lit,
        "l_mal_lit": l_mal_lit,
        "c_galosn_lit": c_galosn_lit,
        "c_zychn_lit": c_zychn_lit,
        "c_vialik_lit": c_vialik_lit,
        "c_mal_lit": c_mal_lit,
    }
    ...
    class ctlr_g(bs.Rule):
        ...
        def work_with(self, text, regexp_rule):
            ...
            for i in self.karani:
                cur_state = 0
                while (
                    re.search(i.replace("@", "г"), result[cur_state:], re.IGNORECASE)
                    != None
                ):
                    cur_state = re.search(
                        i.replace("@", "г"), result[cur_state:], re.IGNORECASE
                    ).start()
                    # Параўноўваем
                    # @узік
                    # ГуЗіК
                        ...
                        while not re.match(r"\s", result[cur_state + count]):
                            if i[count] == "@":
                                if result[cur_state + count] == "Г":
                                    result = (
                                        result[: cur_state + count]
                                        + "G"
                                        + result[(cur_state + count + 1) :]
                                    )
    ...
    class ctlr_je_jo_ju_ja(bs.Rule):
        ...
        def work_with(self, text, regexp_rule):
            je_regexp = (
                "(?<=["
                + self.gram_baza["l_galosn_lit"]
                + self.gram_baza["c_galosn_lit"]
                + r"'ўЎ\s\n])"
            )
            ie_regexp = (
                "(?<=["
                + self.gram_baza["l_zychn_lit"]
                + self.gram_baza["c_zychn_lit"]
                + "])(?<![ўЎлЛŭǓlL])"
            )
            e_regexp = "(?<=[лЛlL])"
            ...
            # З улікам суседніх літар, т.е БЕРАСТ павінен быць BIERAST, а не BIeRAST
                ...
                vialik_lit = (
                    "["
                    + self.gram_baza["l_vialik_lit"]
                    + self.gram_baza["c_vialik_lit"]
                    + "]"
                )
                while "J" + mtch in e_res[cur_st:]:
                    cur_st = e_res.index("J" + mtch, cur_st)
                    try:
                        if re.match(vialik_lit, e_res[cur_st - 1]) or re.match(
                            vialik_lit, e_res[cur_st + 2]
                        ):
                            e_res = (
                                e_res[: cur_st + 1] + mtch.upper() + e_res[cur_st + 2 :]
                            )
                    except IndexError:
                        continue
                    cur_st += 2
        ...
        def work_with(self, text, regexp_rule):
            if regexp_rule == "х":
                ...
                while "Ch" in ch_text[cur_st:]:
                    ...
                    try:
                        if re.match(vialik_lit, ch_text[cur_st - 1]) or re.match(
                            vialik_lit, ch_text[cur_st + 2]
                        ):
                            ch_text = (
                                ch_text[: cur_st + 1] + "H" + ch_text[cur_st + 2 :]
                            )
                    except IndexError:
                        continue
                    cur_st += +2
    ...
    class ctlr_assimil_pa_miahk(bs.Rule):
        ...
        assim_para = {"Z": "Z", "z": "ź", "S": "S", "s": "ś", "C": "C", "c": "ć"}

        assim_para_n_l = {"N": "Ń", "n": "ń", "Ł": "L", "ł": "l"}
        ...
        def work_with(self, text, regexp_rule):
            ...
            for i in self.assim_para.keys():
                ...
                result = re.sub(
                    i + "(?=[" + self.zmiahch + "])(?!(Ch|CH|ch|cH|K|k|G|g|H|h))",
                    aspi,
                    result,
                )
                result = re.sub(
    ...
    ctl_rules = {
        ...
        "Х": ctrl_ch(gram_baza),
        "х": ctrl_ch(gram_baza),
        "Е": ctlr_je_jo_ju_ja(gram_baza),
        "е": ctlr_je_jo_ju_ja(gram_baza),
        "Ё": ctlr_je_jo_ju_ja(gram_baza),
        "ё": ctlr_je_jo_ju_ja(gram_baza),
        "Ю": ctlr_je_jo_ju_ja(gram_baza),
        "ю": ctlr_je_jo_ju_ja(gram_baza),
        "Я": ctlr_je_jo_ju_ja(gram_baza),
        "я": ctlr_je_jo_ju_ja(gram_baza),
        "З": "Z",
        "з": "z",
        "Н": "N",
    ...
    def cyr_to_lat(self, text_in):
        ...
        for character in self.ctl_rules.keys():
            ...
            if isinstance(self.ctl_rules[character], str):
                ...
                result = self.ctl_rules[character].work_with(
                    text=result, regexp_rule=character
                )
        return result
